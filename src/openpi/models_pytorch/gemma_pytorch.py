...
class PaliGemmaWithExpertModel(nn.Module):
    """π0 机器人策略的双流 Transformer 模型。

    架构包含两个并行的语言模型流：
      - paligemma (前缀流)：视觉-语言模型，负责处理图像特征和自然语言 token。
      - gemma_expert (后缀流)：纯语言模型，作为"动作专家"，负责处理动作 token。

    在联合推理时，两个流每一层共享注意力计算（将 Q/K/V 拼接后统一做
    attention，再拆分回各自的 hidden states），实现跨流信息交互，最终由
    gemma_expert 的输出预测动作。

    AdaRMS 是自适应 RMS LayerNorm，可接受外部条件向量（如扩散噪声时间步
    的 embedding）对归一化进行调制，use_adarms 分别控制两个流是否启用。
    """
    ...
    def __init__(
        ...
        # ---- 构建 Gemma Expert (动作专家，后缀流) 的 HuggingFace 配置 ----
        ...
        # ---- 分支 3：联合推理（前缀 + 后缀同时存在，共享注意力） ----
            ...
            def compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond):
                ...
                for i, hidden_states in enumerate(inputs_embeds):
                    ...
                    # input_layernorm 可能返回 (normed, gate)，gate 用于 AdaRMS 残差缩放
                ...
                # Step 3：在序列（token）维度上拼接两个流的 Q/K/V，实现跨流注意力
